          ...
          <div>
            ...
            <ul className="space-y-2">
              {countries?.slice(0, 6).map((country) => (
                <FooterCountryCities key={country.code} countryCode={country.code} />
              ))}
            </ul>
          </div>
...
function FooterCountryCities({ countryCode }: { countryCode: string }) {
  const { t } = useTranslation();
  const { data: cities } = useCities(countryCode);

  // Show first 2 cities per country in footer
  ...
  return (
    <>
      {topCities.map((city) => (
        <li key={`${countryCode}-${city.name}`}>
          <Link
            to={`/city/${countryCode}/${encodeURIComponent(city.name)}`}
            className="text-sm text-muted-foreground hover:text-foreground transition-colors"
          >
            {city.name}, {t(`countries.${countryCode}`, { defaultValue: countryCode })}
          </Link>
        </li>
      ))}
    </>
