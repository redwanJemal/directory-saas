You are implementing task "Logging Infrastructure — Structured JSON, Tracing" for the Directory SaaS boilerplate project.
...
5. After ALL work is done, update the progress file at docs/tasks/progress.json:
   - Set tasks.06-logging-infrastructure.status to "completed"
   - Set each subtask to "completed" as you finish them
   - Set tasks.06-logging-infrastructure.completed_at to current ISO timestamp
   - Add any important notes to tasks.06-logging-infrastructure.notes
6. Finally, create a git commit with message: "feat: implement 06-logging-infrastructure — Logging Infrastructure — Structured JSON, Tracing"
