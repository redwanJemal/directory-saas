You are implementing task "PostgreSQL Row-Level Security Policies" for the Directory SaaS boilerplate project.
...
5. After ALL work is done, update the progress file at docs/tasks/progress.json:
   - Set tasks.08-postgresql-rls.status to "completed"
   - Set each subtask to "completed" as you finish them
   - Set tasks.08-postgresql-rls.completed_at to current ISO timestamp
   - Add any important notes to tasks.08-postgresql-rls.notes
6. Finally, create a git commit with message: "feat: implement 08-postgresql-rls — PostgreSQL Row-Level Security Policies"
