You are implementing task "Background Jobs — BullMQ + Redis" for the Directory SaaS boilerplate project.
...
5. After ALL work is done, update the progress file at docs/tasks/progress.json:
   - Set tasks.16-background-jobs.status to "completed"
   - Set each subtask to "completed" as you finish them
   - Set tasks.16-background-jobs.completed_at to current ISO timestamp
   - Add any important notes to tasks.16-background-jobs.notes
6. Finally, create a git commit with message: "feat: implement 16-background-jobs — Background Jobs — BullMQ + Redis"
