You are implementing task "Health Checks & Readiness Probes" for the Directory SaaS boilerplate project.
...
5. After ALL work is done, update the progress file at docs/tasks/progress.json:
   - Set tasks.20-health-checks.status to "completed"
   - Set each subtask to "completed" as you finish them
   - Set tasks.20-health-checks.completed_at to current ISO timestamp
   - Add any important notes to tasks.20-health-checks.notes
6. Finally, create a git commit with message: "feat: implement 20-health-checks — Health Checks & Readiness Probes"
