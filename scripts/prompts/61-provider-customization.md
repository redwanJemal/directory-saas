You are implementing task "Provider Portal — Habesha Customization" for the Habesha Hub project (Ethiopian Business Directory for the Middle East).
...
These subtasks have already been tracked:
  ...
  - deals-manager: pending
  - contact-analytics: pending
  - dashboard: pending
...
5. After ALL work is done, update the progress file at docs/tasks/progress.json:
   - Set tasks.61-provider-customization.status to "completed"
   - Set each subtask to "completed" as you finish them
   - Set tasks.61-provider-customization.completed_at to current ISO timestamp
   - Add any important notes to tasks.61-provider-customization.notes
6. Finally, create a git commit with message: "feat: implement 61-provider-customization — Provider Portal — Habesha Customization"
